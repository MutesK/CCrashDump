...
CAPIHook::CAPIHook(PSTR pszCalleeModName, PSTR pszFuncName, PROC pfnHook, bool bThisMod) 
{
	...
	//   char* pszExeFile = wcsrchr(szPathname, L'\\') + 1;
	...
	ReplaceIATEntryInAllMods(m_pszCalleeModName, m_pfnOrig, m_pfnHook, bThisMod);
}
...
void CAPIHook::ReplaceIATEntryInAllMods(PCSTR pszCalleeModName, PROC pfnCurrent, PROC pfnNew, bool bThisMod) 
{
	...
	for (; pImportDesc->Name; pImportDesc++) 
	{
		...
		if (lstrcmpiA(pszModName, pszCalleeModName) == 0) 
		{
			...
			if (bFound) 
			{
				...
				{
					DWORD dwOldProtect;
					if (VirtualProtectEx(GetCurrentProcess(), ppfn, sizeof(pfnNew), PAGE_READWRITE, 
						&dwOldProtect)) 
					{

						WriteProcessMemory(GetCurrentProcess(), ppfn, &pfnNew, 
						sizeof(pfnNew), NULL);
						VirtualProtectEx(GetCurrentProcess(), ppfn, sizeof(pfnNew), dwOldProtect, 
						&dwOldProtect);
					}
				}
